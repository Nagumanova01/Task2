...
{
    ifstream in(path);
    ofstream out1("fm1.txt");
    out1.close();
    ofstream out2("fm2.txt");
    out2.close();
    int tmp = 1;
    string film;
    bool f1 = 1, f2 = 1;
    while (1)
    {
        ofstream out1("fm1.txt", ios_base::app);
        for (int i = 1;i <= k;i++)
        {
            //in >> num;
            if (getline(in, film))
        ...
        out1.close();
        ofstream out2("fm2.txt", ios_base::app);
        for (int i = 1;i <= k;i++)
        {
            if (getline(in, film))
            {
...
{
    int tmp = 0,x;
    ifstream in1("fm1.txt");
    ifstream in2("fm2.txt");
    ofstream out(path);
